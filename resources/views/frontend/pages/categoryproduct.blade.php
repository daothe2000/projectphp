      ...
      <div class="col-lg-9 col-md-6 col-sm-12 product-de-1">
        <div class="Simplesearch">
          <form class="search-box-inner" action="{{ route('search') }}" method="get">
            <input type="text" id="input" name="search" placeholder="Search your products">
            <button id="button" type="submit">Search</button>
            <form>
            </div>
            <div class="row product">
              @foreach($category_pro as $cat_pro)
              <div class="col-lg-4 col-sm-12 get-item">
                <div class="pro1">
                  <div class="img-pro">
                    <a href="{{route('product_detail',['id'=>$cat_pro->id])}}"><img src="{{asset('public/frontend/imgs/'.$cat_pro->image)}}" alt=""></a>
                  </div>

                  <div class="text-pro">
                    <a href="{{route('product_detail',['id'=>$cat_pro->id])}}"><p>{{$cat_pro->pro_name}}</p></a>

                    <p>{{$cat_pro->cat->name}}</p>

                    <strong>{{number_format($cat_pro->price)}}đ</strong>
                  </div>
                  <div class="cart">
                   <a  data-url="{{route('add-cart',['id'=>$cat_pro->id])}}" class="add-cart2"> <button class="btn">ADD TO CART<i class="fas fa-plus text-center"></i></button></a>
                 </div>
               </div>
             </div>
             @endforeach
             <div class="col-12 paginationpro"><div class="row">{{ $category_pro->links() }}</div></div>
           </div>
         </div>
